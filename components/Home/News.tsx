...
const News = (props: INews) => {
	const [news, setNews] = useState<{ id: number, title: string; description: string, img: string }[]>([]);
	...
	return (
		<div className="w-full h-full flex flex-col space-y-4">
			{news.map((newsPost) => (
				<div key={newsPost.id} className={`w-4/5 p-8 flex space-x-4 max-h-[20rem] ${props.dark ? "border border-brand-font shadow-light-btn" : "border border-brand-black shadow-btn"}`}>
